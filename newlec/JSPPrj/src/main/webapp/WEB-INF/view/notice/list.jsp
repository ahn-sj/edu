...
<body>
		...
		<main class="main">
			...
			<div class="search-form margin-top first align-right">
				...
				<form class="table-form">
					<fieldset>
						...
						<select name="f">
						<%-- 검색한 이후에 제목/작성자도 유지시키기 위해 EL로 삼항연산를 사용 --%>
							<option ${(param.f == "title")?"selected":""} value="title">제목</option>
							<option ${(param.f == "writer_Id")?"selected":""} value="writer_Id">작성자</option>
						</select> 
						<label class="hidden">검색어</label>
						<%-- value에 param.q를 넣어주면 검색한 내용이 유지된다 --%>
			...
			<div class="notice margin-top">
				...
				<table class="table">
					...
					<%-- 
						공개 설정이 된 데이터만 사용자가 확인할 수 있도록 하기 위해선
						사용자 페이지가 아니라 서버에서 처리를 해주어야 한다
						그러지 않으면 빈 10개의 값을 받기때문에 비효율적이다						
					 --%>
			...
			<div class="indexer margin-top align-right">
				...
				<%-- empty 연산자는 null과 ""(빈문자열)을 체크해준다 --%>
				<%-- Pager에서 현재 페이지 번호 처리 (1) --%>
	...
	<ul class="-list- center">
		...
		<%-- 전체페이지를 조건에 따라 번호를 표시할지 말지  --%>
		<c:if test="${(startNum+i) <= lastNum }">
		<%-- Pager에서 현재 페이지 번호 처리 (3) --%>
